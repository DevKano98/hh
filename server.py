...
async def detect_face_endpoint(file: UploadFile = File(...)):
    ...
    res = svc.detect_and_embed(img_cv)
    if res == "MULTIPLE_FACES":
        raise HTTPException(status_code=400, detail="Multiple faces detected. Please upload an image with exactly one authorized person.")
    if not res:
        return {
            "status": "success",
            "detected": True,
    ...
    session_state["ref_embedding"] = res["embedding"]
    session_state["ref_bbox"] = res["bbox"]
    session_state["ref_confidence"] = res["confidence"]
